...
<script type="text/javascript">
		...
		$(document).on("click","#end_work", function(){
			...
			Swal.fire({
				   ...
				   if (result.isConfirmed) { // 만약 모달창에서 confirm 버튼을 눌렀다면
					  ...
					  $.ajax({
						  url:"<%= ctxPath %>/commute/commuteEnd.pm",
						  type:"POST",
						  data:{"fk_empno":'${sessionScope.loginuser.empno}',
							    "worktime":worktime,
							    "overtime":overtime},
    ...
    function logout(){
    	location.href='<%= ctxPath%>/logout.pm';
    }
	...
	function getUnreadMsgCnt(){
		...
		$.ajax({
	    	url : "<%=ctxPath%>/messenger/getUnreadMsgCnt.pm",
	    	dataType:"JSON",
			success: function(json){
	...
	function getUnreadAlarmCnt(){
		...
		$.ajax({
	    	url : "<%=ctxPath%>/alarm/getUnreadAlarmCnt.pm",
	    	dataType:"JSON",
			success: function(json){
		...
		<div id="sideBar">
			...
			<div id="sideTop" class="border-bottom">
				...
				<div id="div_team" class="dropdown-menu">
					<button class="dropdown-item" type="button" onclick="javascript:location.href='<%= ctxPath%>/userDetail.pm?empno=${sessionScope.loginuser.empno}'">내 정보</button>
				...
				<div class="sideTr" onclick="javascript:location.href='<%= ctxPath%>/messenger/receivedMessage.pm'">
			...
			<div id="sideMiddle" class="border-bottom">
				<div class="sideTr" onclick="javascript:location.href='<%= ctxPath%>/notice/noticeList.pm'">
					<i class="fas fa-regular fa-flag sideIcon"></i><span>공지사항</span>
				</div>
				<div class="sideTr" onclick="javascript:location.href='<%= ctxPath%>/people.pm'">
				...
				<div class="sideTr" onclick="javascript:location.href='<%= ctxPath%>/commute/mycommute.pm'">
					<i class="fas fa-regular fa-clock sideIcon"></i><span>출퇴근</span>
				</div>
				<div class="sideTr" onclick="javascript:location.href='<%= ctxPath%>/leaveSummary.pm'">
				...
				<div class="sideTr" onclick="javascript:location.href='<%= ctxPath%>/admin/severance_pay.pm'">
	               <i class="fas fa-solid fa-comments-dollar sideIcon"></i><span>퇴직금</span>
	            </div>
	            </c:if>
				<div class="sideTr" onclick="javascript:location.href='<%= ctxPath%>/insight.pm'">
