...
<script>
	...
	arr_status = [];
		...
		$(document).ready(function(){
			...
			$("button#changememo").click(function(){
				const frm = document.frm_memo;
	 			frm.method="POST";
	 			frm.action="<%= ctxPath%>/changePsaMemo.pm";
	 			frm.submit();
			});
		...
		function func_getEmpInfo(empno){ <%-- 파라미터로 사원 번호 전달 받기 --%>
		 	<%-- 특정 사원번호 전달 --%>
			location.href = "<%=ctxPath%>/userDetail.pm?empno="+empno;
		}
		...
		function makeCommentPageBar(currentShowPageNo){
			...
			$.ajax({
				url:"<%=request.getContextPath()%>/getTotalPsaPage.pm",
				data:{"sizePerPage":"10"
					 ,"keyword":keyword
		...
		function viewEmpList(currentShowPageNo){
			...
			$.ajax({
				url:"<%=request.getContextPath()%>/psaListPaging.pm",
...
<div id="peopleContent">
		...
		<div id="header">
	        <div id="header_title">
	            ...
	            <a class="a_title" href="<%= ctxPath%>/organization_chart.pm"><span class="title">조직도</span></a>
	            <c:if test="${sessionScope.loginuser.empno == 9999}">
	            	<a class="current" href="<%= ctxPath%>/change_history.pm"><span class="title">인사 정보 관리</span></a>
	            </c:if>
	        </div>
	        <div id="button_title">
