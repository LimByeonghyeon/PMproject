...
//		$(document).on("change","input#between_date",function(){
		  ...
		  let startdate = $("input#start_date").val();
			...
			if(!changeLeaveFalg){
				$.ajax({
					  ...
					  url : "<%= request.getContextPath()%>/checkLeave.pm",
					  data : {"empno":empno
						  	 ,"startdate":startdate
						  	 ,"enddate":enddate},
					  type : "POST",
		...
		$(document).on("click","div#div_hr", function(e){
			...
			html+=	"</div>"
					...
					if("${requestScope.employeeMap.status}" == "휴직"){
						$.ajax({
							  url : "<%= request.getContextPath()%>/getLeaveInfo.pm",
				  ...
				  success : function(json){
					  if(json.result > 0){
						  ...
						  if(leaveFlag){ // 날짜를 올바르게 입력한 경우에만 휴직 신청가능
						  		...
						  		$.ajax({
									  url : "<%= request.getContextPath()%>/leaveAbsence.pm",
		...
		$("button#btn_search_leaveInfo").on("click",function(){
			...
			$.ajax({
				  url : "<%= ctxPath%>/getLeaveAbsence.pm",
				  data: {"empno":empno},
				  dataType : "JSON",
				  success : function(json){
				  		$("div#div_result").html(""); 
		...
		$(document).on("click","button#deleteLeave",function(){
			Swal.fire({
				   ...
				   if (result.isConfirmed) { // 만약 모달창에서 confirm 버튼을 눌렀다면
					   ...
					   $.ajax({
							  url : "<%= request.getContextPath()%>/cancelLeave.pm",
							  data : {"empno":"${requestScope.employeeMap.empno}"
								  	 ,"leaveno":leaveno},
							  type : "POST",
							  dataType : "JSON",
					  ...
					  if(json.result > 0){
						  ...
						  Swal.fire({
							   ...
							   if (result.isConfirmed) { // 만약 모달창에서 confirm 버튼을 눌렀다면
							   		...
							   		frm.action = "<%= request.getContextPath() %>/retirement.pm";
							   		frm.method = "POST";
							   		frm.submit();
							   }
							});
	...
	function changePsInfo(){
		  ...
		  frm.action="<%= ctxPath%>/changePsInfo.pm";
		  frm.method="POST";
		  frm.submit();
	}
	...
	function func_psa(){
		...
		frm.action="<%= ctxPath%>/personnelAppointment.pm";
		frm.method="POST";
		frm.submit();
	}
   ...
   function getCurrentWeek() { 
	...
	function getFile(empno){
		$.ajax({
			  url : "<%= request.getContextPath()%>/getFile.pm",
			  data : {"empno":empno},
			  type : "POST",
			  dataType : "JSON",
			  success : function(json){
				  if(json.length>0){
						...
						$.each(json, function(index, item){
							html+='<span style="font-size: 10pt; color: gray;"><i class="fas fa-solid fa-paperclip ml-3 mr-1"></i></span>'+
					        	'<span class="mailFiles" onclick="javascript:location.href=\'<%=ctxPath%>/downloadFile.pm?filename='+item.filename+'&org_filename='+item.org_filename+'\'" >'+item.org_filename+'</span><br>';
						});
...
<div id="user_detail_content">
	...
	<span><a class="a_people" href="<%= ctxPath%>/people.pm">구성원</a> / ${requestScope.employeeMap.name}</span>
			...
			<thead>
				...
				<tr>
					<th colspan="3">
						...
						<a class="btn communication" href="<%= ctxPath%>/messenger/receivedMessage.pm?empno=${requestScope.employeeMap.empno}" data-toggle="tooltip" data-placement="top" title="메신저"><i class="fas fa-comment"></i></a>
					</th>
					<th> 
	...
	<div style="display:flex; justify-content: space-between;">
		...
		<div id="right_sidebar">
			...
			<a class="a_side" href="<%= ctxPath%>/commute/mycommute.pm">
			...
			<a class="a_side" href="<%= ctxPath%>/leaveSummary.pm">
			...
			<a class="a_side" href="<%= ctxPath%>/admin/payStub.pm">
