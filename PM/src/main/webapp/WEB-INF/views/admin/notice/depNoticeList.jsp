...
<script type="text/javascript">
		...
		// 공지글 삭제하기 버튼 클릭시 
      	$(document).on("click","button.DeleteBnt",function(){
	         ...
	         */
          	 ...
          	 $.ajax({
	             url:"<%= request.getContextPath() %>/notice/deleteDepNoticeEnd.pm",
	             type : "POST",
	             data : {"notino" : notino,
	...
	//	console.log("확인용 공지번호 "+notino);
		...
		$.ajax({
	    	url : "<%= request.getContextPath() %>/notice/deptOneNoticeContent.pm",
	    	type: 'POST',
	    	data : {"notino" : notino},
	    	dataType: "JSON",
			success: function(json){
			//	console.log(json);
				...
				// console.log(json.fileName);
				 ...
				 // console.log(file_notino);
					$.ajax({
				    	url : "<%= request.getContextPath() %>/notice/getDepFile.pm",
							...
							if(json2.length>0){
								...
								$.each(json2, function(index2, item2){
									html+='<span style="font-size: 10pt; color: gray;"><i class="fas fa-solid fa-paperclip ml-3 mr-1"></i></span>'+
							        	'<span id="notifile"  style="cursor:pointer;" onclick="javascript:location.href=\'<%= request.getContextPath() %>/notice/downloadDepFile.pm?fileName='+item2.fileName+'&orgFilename='+item2.orgFilename+'\'" >'+item2.orgFilename+'</span><br>';
								});
	...
	function openNoticeEditModal(notino) {
			...
			$.ajax({
		    	url : "<%= request.getContextPath() %>/notice/getDepNoticeContent.pm",
		    	type: 'POST',
		    	data : {"notino" : notino},
		    	dataType: "JSON",
				success: function(json){
		...
		<c:forEach var="deptNoti" items="${requestScope.depNoticeList}">
			<div class="listRow">
				<div class="listRowInside" style="width: 100%;">
					...
					<button type="button" class="listBnt EditBnt" style="background-color: white; color: #07b419; margin-left: 620px; float: right;"  data-toggle="modal" data-target=".noticeEditModal">수정하기</button>
