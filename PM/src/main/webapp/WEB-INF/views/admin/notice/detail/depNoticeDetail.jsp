...
<script type="text/javascript">
	...
	//		console.log(notino);
			...
			Swal.fire({
				...
				//		console.log(notino);
						...
						// 댓글 삭제 버튼 누를 경우
						$.ajax({
					    	url : "<%= request.getContextPath() %>/notice/delDepComment.pm",
					    	type: 'POST',
    ...
    $(document).on("click","button.cmtEditBnt", function(){
    	...
    	if(content == ""){
    		...
    		// 댓글 내용 수정 후 확인 누르면 
    		$.ajax({
		    	url : "<%= request.getContextPath() %>/notice/editDepComment.pm",
		    	type: 'POST',
	...
	function goAddCmtReal() {
   		...
   		$.ajax({
		  url:"<%= request.getContextPath()%>/notice/addDepComment.pm",
		  data: {//cmtForm,
	...
	//	console.log(fk_notino);
		$.ajax({
   			url:"<%= request.getContextPath()%>/notice/readDepComment.pm",
