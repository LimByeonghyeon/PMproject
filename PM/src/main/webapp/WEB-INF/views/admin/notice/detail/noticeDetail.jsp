...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		// 페이징 처리 안한 댓글 읽어오기 // 문서로딩되자마자 이 함수 호출
		// goReadComment();
	    ...
	    $(document).on("click","button.cmtEditBnt", function(){
	    	...
	    	let notino = $(this).parent().parent().prev().find("#cmt_notino").val();
		    	...
		    	if(content == ""){
		    		...
		    		// 댓글 내용 수정 후 확인 누르면 
		    		$.ajax({
				    	url : "<%= request.getContextPath() %>/notice/editComment.pm",
				    	type: 'POST',
		...
		//		console.log(notino);
				...
				Swal.fire({
					...
					//		console.log(notino);
							...
							// 댓글 삭제 버튼 누를 경우
							$.ajax({
						    	url : "<%= request.getContextPath() %>/notice/delComment.pm",
						    	type: 'POST',
	...
	function goAddCmtReal() {
   		...
   		$.ajax({
		  url:"<%= request.getContextPath()%>/notice/addComment.pm",
		  data: {//cmtForm,
	...
	//	console.log(fk_notino);
		$.ajax({
   			url:"<%= request.getContextPath()%>/notice/readComment.pm",
   	...
   	<div class="modalBody">
  	 	<div class="modal-listContent"> 
		    	...
		    	<form id="commentFrm" name="commentFrm" style="resize: none; width:100%; margin: 0;">
		       	<textarea id="commentContent" name="content" maxlength="50" placeholder="댓글을 입력하세요." style="width:100%; height: 20%; margin: 0;"></textarea>
		       	<div class="mt-2">
