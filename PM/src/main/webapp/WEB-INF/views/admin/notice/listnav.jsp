...
// 	let flag = false;
	...
	$(document).ready(function() {
		...
		$("button#sendNotiBtn").click(function(){
			...
			$.ajax({
		    	url : "<%= request.getContextPath() %>/notice/sendNotice.pm",
		    	data : queryString,
		    	type: 'POST',
   ...
   function openAjax(){
      $.ajax({
          url : "<%= request.getContextPath() %>/notice/getDept.pm", // http://localhost:9090/hr/notice/getDept.pm
          dataType: "JSON",
          async:false,
	...
	function goSearch(){
		...
		location.href = "<%=ctxPath%>/notice/noticeList.pm?searchWord="+searchWord;
	}
	--%>
...
<div id="noticeNav">
	...
	<div id="bottomcate" style="float: none;">
		<span id="cate1" style="font-size: 13pt; font-weight: bold;" onclick="javascript:location.href='<%= ctxPath%>/notice/noticeList.pm'">전체 공지</span>
		&nbsp; &nbsp; 
		<span id="cate2" style="font-size: 13pt; font-weight: bold;" onclick="javascript:location.href='<%= ctxPath%>/notice/depNoticeList.pm'">부서 공지</span>
		&nbsp; &nbsp; 
		<span id="cate3" style="font-size: 13pt; font-weight: bold;" onclick="javascript:location.href='<%= ctxPath%>/notice/myNoticeList.pm'">내가 쓴 공지</span>
