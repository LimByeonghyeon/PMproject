...
<script type="text/javascript">
		...
		// 공지글 삭제하기 버튼 클릭시 
      	$(document).on("click","button.DeleteBnt",function(){
	         ...
	         */
          	 ...
          	 $.ajax({
	             url:"<%= request.getContextPath() %>/notice/deleteNoticeEnd.pm",
	             type : "POST",
	             data : {"notino" : notino,
	...
	//	console.log(searchWord);
		...
		$.ajax({
	    	url : "<%= request.getContextPath() %>/notice/getNoticeContent.pm",
	    	type: 'POST',
	    	data : {"notino" : notino},
	    	dataType: "JSON",
			success: function(json){
				...
				$("#myListModal input#fk_notino").val(json.notino); // 댓글 fk_notino
					...
					//	console.log(json.fileName);
					 ...
					 console.log(file_notino);
						$.ajax({
					    	url : "<%= request.getContextPath() %>/notice/getFile.pm",
								...
								if(json2.length>0){
									...
									$.each(json2, function(index2, item2){
										html+='<span style="font-size: 10pt; color: gray;"><i class="fas fa-solid fa-paperclip ml-3 mr-1"></i></span>'+
								        	'<span id="notifile"  style="cursor:pointer;" onclick="javascript:location.href=\'<%= request.getContextPath() %>/notice/downloadNoticeFile.pm?fileName='+item2.fileName+'&orgFilename='+item2.orgFilename+'\'" >'+item2.orgFilename+'</span><br>';
									});
	...
	function goSearch() {
		...
		frm.action = "<%= request.getContextPath() %>/notice/noticeList.pm"; 
									// 자기 자신한테로 페이지 이동 
		frm.submit();
